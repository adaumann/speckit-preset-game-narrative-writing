---
description: Compile node files to playable output format (HTML for Twine/SugarCube, compiled JSON for Ink with HTML wrapper, etc.). First creates a combined compile.twee with [MECHANIC:...] token conversion, then runs postprocessing (header injection, animation, theme) before the native compiler. Prefers export/ over draft/ when export exists. Includes built-in structural validation and a self-correcting fix loop.
handoffs:
...
- If no matching image file is found for a node, skip silently
...
**Compile steps**:
1. Read all source files in `specs/<specname>/draft/sugarcube/`:
