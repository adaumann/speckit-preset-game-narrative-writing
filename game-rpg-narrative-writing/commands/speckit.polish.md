...
- Store in SESSION object for use in "Load context" step

**RPG Context Loading** (conditional on `SESSION.is_rpg = true`):
- **Tabletop**: Read `specs/mechanics-[RULESET].md` (if present), `specs/npc-roster.md`, `specs/craft-rules.md`
- **Computer**: Read `specs/locations.md`, `specs/characters/[PLAYSTYLE]` (Combat/Dialogue/Exploration focus), accessibility variants documentation
...
3. **Load context**:
   ...
   - **Tabletop RPG Context** (if `SESSION.is_rpg = true` and `SESSION.platform = "tabletop"`):
     - Read `specs/mechanics-[RULESET].md` (if present): Ruleset-specific terminology (D&D 5e ability names, Pathfinder 2e degree of success language, Shadowrun 6e matrix terms), example phrasing for mechanical outcomes
     - Read `specs/npc-roster.md`: NPC list with assigned session appearances and narrative voice expectations (referee voice for GM narration nodes)
   - **Computer Game Context** (if `SESSION.is_rpg = true` and `SESSION.platform = "computer"`):
     - Identify playstyle routing from node frontmatter `playstyle: [Combat|Dialogue|Exploration]`
     - Read `specs/characters/[PLAYSTYLE]` guidance (if present): dialogue tone for playstyle (Combat = urgent/authoritative, Dialogue = conversational/diplomatic, Exploration = contemplative/curious)
     - Check for accessibility variants documentation: UI text clarity, color-blind safe descriptions, audio alternative guidance
   ...
   - **RPG-Specific Register** (if `SESSION.is_rpg = true`):
     - Tabletop: Ensure GM narration uses referee voice (neutral, fair, descriptive; not player-character perspective). Ensure NPC dialogue matches assigned character register from character profile.
     - Computer: Ensure narrator tone matches playstyle (Combat nodes use active/urgent language; Dialogue nodes use conversational register; Exploration nodes use observant/curious register). Check for consistency across playstyle branching.
...
4. **Run the polish audit** — scan the node prose for each issue category and record every instance:
   ...
   **TR — Tabletop RPG Prose** (if `SESSION.is_rpg = true` and `SESSION.platform = "tabletop"`)
   - TR-001: GM narration voice drift — nodes with non-choice narration use player-character perspective or emotional interiority (should be referee voice: neutral, fair, descriptive)
   - TR-002: NPC register drift — NPC dialogue vocabulary or tics inconsistent with character profile (vocabulary pool, verbal tics from `specs/characters/[NPC_ID].md`)
   - TR-003: Ruleset terminology — mismatched ability names (D&D 5e), degree of success language (PF2e), or dice notation (SR6e) inconsistent with ruleset standards
   - TR-004: Combat narration clarity — combat prose unclear about action sequence, positioning, or outcome (who did what to whom)
   - TR-005: Mechanical outcome phrasing — skill check outcomes use passive voice or lack NPC reaction (e.g., "The lock opened" vs. "Corvus twisted the pick and the lock clicked open — he was in")

   **CR — Computer Game Prose** (if `SESSION.is_rpg = true` and `SESSION.platform = "computer"`)
   - CR-001: Playstyle tone inconsistency — narrator voice shifts register across playstyle branches (e.g., Combat nodes urgent, Dialogue nodes conversational, but within one branch tone is unstable)
   - CR-002: UI text clarity — text describing interactive elements is vague or uses undefined terms (e.g., "examine the object" should not appear; instead specify what's visible)
   - CR-003: Accessibility language — descriptions of colors, audio, or physical actions lack alternatives for colorblind/deaf/motor-impaired accessibility
   - CR-004: Route convergence prose — text at convergence points differs significantly in register or emotional temperature across incoming routes (should feel seamless)
...
5. **Present the Polish Audit Report** before making any edits:
   ...
   **Tabletop RPG Prose Report** (if `SESSION.is_rpg = true` and `SESSION.platform = "tabletop"`):
   ```
   ## Polish Audit: <NODE_ID> [Tabletop RPG]

   **Session**: S-[N] | **NPCs**: Corvus (2 dialogue lines), Marcus (1 dialogue line) | **GM Narration**: Yes
   **Ruleset**: [D&D 5e|Pathfinder 2e|Shadowrun 6e]

   | Issue ID | Category | Location | Issue | Proposed fix |
   |---|---|---|---|---|
   | TR-001 | GM Voice | Para 2 | "I felt the cold stone beneath my hands" — player perspective in GM narration | "The stone was cold and damp." (referee voice) |
   | TR-002 | NPC Register | Line 8 | "The Shadowrunner used sophisticated language" — Corvus is street-slang register | Rewrite in street slang |
   | TR-003 | D&D 5e | Para 4 | "roll a difficulty check" | "make an Intelligence check" |
   | TR-004 | Combat | Para 6 | "Combat happened" — unclear action sequence | "Corvus drew his sword and lunged left; Marcus rolled under the blade" |
   | PR-001 | Rhythm | Para 3, sentences 2–6 | 5 sentences averaging 12 words | Vary: split sentence 4; merge 5+6 |

   Total issues: N (TR: N | CR: N | PR: N | WR: N | VR: N | DI: N)
   Estimated change surface: N sentences / N words affected
   ```

   **Computer Game Prose Report** (if `SESSION.is_rpg = true` and `SESSION.platform = "computer"`):
   ```
   ## Polish Audit: <NODE_ID> [Computer Route]

   **Route**: [Combat|Dialogue|Exploration] | **Convergence Point**: No | **Accessibility Variants**: 3 (colorblind, subtitle, motor)

   | Issue ID | Category | Location | Issue | Proposed fix |
   |---|---|---|---|---|
   | CR-001 | Playstyle Tone | Para 2 | Dialogue-route text uses urgent language ("danger") — should be conversational | Change to diplomatic/calm tone |
   | CR-002 | UI Clarity | Line 5 | "interact with the interface" — too vague | "The glowing terminal shows two options: [AUTHORIZE] and [DENY]" |
   | CR-003 | Accessibility | Para 3 | "the red button" — no colorblind alternative | "the large circular button (top-left)" |
   | PR-001 | Rhythm | Para 3, sentences 2–6 | 5 sentences averaging 12 words | Vary: split sentence 4; merge 5+6 |

   Total issues: N (CR: N | PR: N | WR: N | VR: N | DI: N)
   Estimated change surface: N sentences / N words affected
   ```
...
9. **Report**:
   ...
   - **Tabletop RPG**: Confirmation of NPC register consistency and GM narration voice (if present). Campaign prep note if SESSION-[N]-BRIEFING.md or campaign-guide.md prose references this node.
   - **Computer Game**: Confirmation of playstyle tone consistency and accessibility variant parity. Route convergence note if this is a convergence point.
...
**RPG Prose Standards**:
- **Tabletop**: GM narration must use referee voice (neutral, fair, descriptive). Do not inject player-character thoughts, fears, or perspectives into GM narration. If a node includes NPC dialogue, register must match character profile vocabulary pool and verbal tics. Mechanical outcomes should show NPC reaction ("the lock opened and Corvus smirked" not just "the lock opened")
- **Computer**: All playstyle branches of the same node must have consistent emotional register and narrator tone. Dialogue-focused routes use conversational tone; Combat routes use urgent tone; Exploration routes use observant/curious tone. At convergence points, incoming text must feel unified (no register shift).
- **Ruleset**: D&D 5e uses ability names exactly (Strength, not Power). Pathfinder 2e uses degree of success terminology (critically succeeds, succeeds, fails, critically fails). Shadowrun 6e distinguishes hacker jargon (matrix terms) from street slang (character register). Maintain terminology consistency within the ruleset.
...
- Tabletop campaign prep documents (campaign-guide.md, SESSION-N-BRIEFING.md, campaign-pacing-guide.md) → use `speckit.revise` for those documents
